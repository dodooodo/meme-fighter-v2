...
## Required Change
Add `CharacterDetailModel`, resolving each move's animation through the same runtime path the battle uses, and a detail scene listing the character's moves with frame data and an animation preview rendered by the character's own `fighter_visual_scene`. The preview loops, exposes every built frame as a thumbnail strip, and can be paused and stepped one frame at a time. Add a `MOVE LIST` entry point to the mode select screen.
## Public/API Contract
...
9. Playback speed is selectable from 0.1x to 4x, matching the range `ProductionFighterVisual` clamps to. Changing speed keeps the current play state: a paused preview stays paused, because `set_preview_mode` restarts playback and would otherwise silently un-pause a frame somebody stopped to look at.
10. The preview is framed automatically: one scale per character taken from its largest drawn frame, centred on the selected animation, filling the preview without the character floating in empty space.
11. `bash scripts/verify.sh` passes.
12. MANUAL, OUTSTANDING: a human opens the screen and confirms the preview is framed and scaled sensibly, the movelist is readable, and the flow back to mode select behaves. Headless probes cover data and node wiring only; nobody has looked at this screen.
